...
def load_image(name):
    '''
    Эта функция зашружает изобоажение, 
    возвращает объект класса Surface
    '''
...
transportC = None  #переход на следующий уровень через какую либо сторону
screen = pygame.display.set_mode(size)
all_sprites = pygame.sprite.Group()

class Player(pygame.sprite.Sprite):
    '''
    Класс реализации персонажа, за которого играют
    '''
    ...
    def __init__(self, group):
        ...
        self.rect.x = 400
        self.rect.y = 400
        self.tik = 0 # переменная, с помошью которой отсчитывается время
        self.lvl = 0
        self.damage = damage
        self.armor = armor
        self.life = life
    ...
    def update(self, args):
        '''
        Эта функция отвечает за движение, анимацию героя,
        за переход на следующий уровень героем (телепортацию),
        выводит на экран показатели героя.
        Здесь происходит проверка на смерть, поражение и выигрыш, 
        а также их "визуализация".
        '''
        global lvl
        global damage
        global armor
        ...
        if running_sprite == True:
            ...
            self.tik += 1 #увеличиваем переменную отсчета
            if self.tik == 41:
                self.tik = 0
            if go_up == 1: # анимация при нажатии W, движения вверх
                if self.tik == 10:
                ...
                if go_fast == 1:
                    ...
                    self.rect.y -= 1
            elif go_down == 1: # анимация при нажатии S, вниз
                if self.tik == 10:
                ...
                if go_fast == 1:
                    ...
                    self.rect.y += 1
            elif go_left == 1: # анимация принажатии A, влево
                if self.tik == 10:
                ...
                if go_fast == 1:
                    ...
                    self.rect.x -= 1
            elif go_right == 1: # анимация при нажатии D, вправо
                if self.tik == 10:
            ...
            if self.rect.x < -32:# производит телепорт на уровень
                transportC = 'l'# задает что бы потом понять с лева телепортироватся 
                self.rect.x = width - 1
                lvl += 1
                fGreen = (105, random.randrange(150, 255, 15), 105)
            elif self.rect.x > width:# производит телепорт на уровень
                transportC = 'r'# задает что бы потом понять с права стороны телепортироватся
                self.rect.x = -31
                lvl += 1
                fGreen = (105, random.randrange(150, 255, 15), 105)
            elif self.rect.y < -46: # производит телепорт на уровень
                transportC = 'u'# задает что бы потом понять с верха телепортироватся
                self.rect.y = height - 1
                lvl += 1
                fGreen = (105, random.randrange(150, 255, 15), 105)
            elif self.rect.y > height:# производит телепорт на уровень
                transportC = 'd'# задает что бы потом понять с низа телепортироватся
                self.rect.y = -45
                lvl += 1
                fGreen = (105, random.randrange(150, 255, 15), 105)#
            ...
            xt1 = 700
            yt1 = 10
            screen.blit(b1, (xt1, yt1)) #  вывод на экран  колво уровней
            ...
            xt3 = 400
            yt3 = 10
            screen.blit(b3, (xt3, yt3)) #  вывод на экран колво  защиты
            
            a4 = pygame.font.Font(None, 25)
            b4 = a4.render(f'Life: {self.life}', 1, RED)
            xt4 = 250
            yt4 = 10
            screen.blit(b4, (xt4, yt4)) #  вывод на экран  колво жизней
            
            a5 = pygame.font.Font(None, 25)
            b5 = a5.render(f'Money: {self.money}$', 1, RED)
            xt5 = 100
            yt5 = 10
            screen.blit(b5, (xt5, yt5)) #  вывод на экран колво денег
        ...
        elif lvl == 21 and money < 40: # проверка на поражение
...
class Chest(pygame.sprite.Sprite):
    ...
    def update(self, args):
        '''
        Эта функция генерит предметы в сундук.
        А также делает переход на следующий уровень
        '''
        global transportC
        global next_l
        global lvl
        ...
        if running_sprite == True:
            ...
            if len(blocks_hit_list) > 1:
                if '[<Player sprite(in 1 groups)>, <Chest sprite(in 1 groups)>]' == str(blocks_hit_list): # проверка на открытие сундука ааааааааааа
                    self.rect.x = -1000
                    self.rect.y = -1000
                    if lvl // 10 == 0: # если левел меньше 10
                        j = random.random() # ПРОСТА ЧИСЛО
                        # далее расматриваются два варинта, с равной степени вероятности выпадения
                        # в зависимлсти от этого происходит генерация предмета в сундуке
                        if j >= 0.5:
                            ...
                            if armor < int(u[1]):   # если защита меньше чем урон предмета
                                ...
                                print(f'you find {lm}$')
                                print('-------------------------')
                    # все анолагично что сверху  только выбираем предмет из более высоких уровней
...
class Skeletron(pygame.sprite.Sprite):
    ...
    def __init__(self, group):
        ...
        self.rect.x = -400
        self.rect.y = -400
        self.tik = 0 # опять левая переменнная, отвечающая за отчет времени
        self.damage = 1
        self.armor = 1
        self.life = 5
        self.no = 0 # Нееееееееееееееееееееееееееееееееееет
    ...
    def update(self, args):
        '''
        Эта функция задает параметры скелета,
        отвечает за битву сос келетом и награждение за убийство.
        Управляет движением скелета.
        '''
        global transportC
        global next_l
        global attack
        ...
        if running_sprite == True:
            ...
            if transportC != None:
                ...
                # вычисления параметров для скелета
                self.damage = (lvl // 5) + 1
                self.armor = (lvl // 10) + 1
                self.life = (lvl // 2) + 5
                next_l += 1
...
# созжание персонажев и пречисление их к общему списку
Player(all_sprites)
Chest(all_sprites)
...
while running:
    clock.tick(fps)
    screen.fill(fGreen)
    pygame.draw.polygon(screen, BLACK, [[40, 40], [760, 40], [760, 760], [40, 760]], 3) # черный полигон
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.KEYDOWN:# дабовляет нажатие клавиш
